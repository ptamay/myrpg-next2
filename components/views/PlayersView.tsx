        ...
        try {
          ...
          if (list.length > 0) {
            if (await showConfirm({ title: "Importar Jogadores", message: `Isso irá adicionar/atualizar ${list.length} jogadores. Continuar?`, type: "warning" })) {
              const currentMap = new Map((players || []).map((n: any) => [n.id, n]));
              list.forEach((n: any) => currentMap.set(n.id, n));
              setPlayers(Array.from(currentMap.values()));
          ...
          <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
            ...
            {isGM && (
              <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
                ...
                <button className="btn primary-btn" onClick={() => { setActiveData(null); openModal('playerForm', null); }}>
