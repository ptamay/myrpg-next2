...
import { useNpcs } from "@/hooks/useGameData";

export default function NpcsView() {
  const { openModal, setActiveData } = useApp();
  const { npcs, setNpcs, loading } = useNpcs();
        ...
        try {
          ...
          if (npcList.length > 0) {
            if (await showConfirm({ title: "Importar NPCs", message: `Isso irá adicionar/atualizar ${npcList.length} NPCs. Continuar?`, type: "warning" })) {
              ...
              const currentMap = new Map(npcs.map((n: any) => [n.id, n]));
              npcList.forEach((n: any) => currentMap.set(n.id, n));
              setNpcs(Array.from(currentMap.values()));
