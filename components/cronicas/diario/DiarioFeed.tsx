...
import { useDiario, useCampaignInfo, usePlayers } from "@/hooks/useGameData";
...
export default function DiarioFeed() {
  ...
  const { jornadaPorDia } = useCampaignInfo();
  ...
  const currentPlayer = players?.find((p: any) => p.id === session?.playerId);
