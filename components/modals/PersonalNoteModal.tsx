...
export default function PersonalNoteModal({ isOpen, onClose }: PersonalNoteModalProps) {
  const { diaAtual, indiceBlocoAtivo, jornadaPorDia, setJornadaPorDia, activeData, salvarEstadoLocal } = useApp();
...
export function PersonalNoteDetailModal({ isOpen, onClose }: PersonalNoteModalProps) {
  const { activeData, setModals, diaAtual, indiceBlocoAtivo, jornadaPorDia, setJornadaPorDia } = useApp();
  const { session } = useUserSession();
